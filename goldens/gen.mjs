/**
 * Golden generator for the euclidean cellular field (seed 42, jitter default 1).
 *
 * Committed and re-runnable: `node goldens/gen.mjs` regenerates
 * `goldens/euclidean.json`. Regenerating is an INTENTIONAL, breaking act -- the
 * digest pins the exact hash constants and placement convention, so any change to
 * `_hash2`/`_hash2b` or the kernel moves it and requires a CHANGELOG note.
 *
 * The unit suite and torture T0 both import { COORDS, SEED, JITTER, digest } from
 * here and re-derive the digest, comparing it against the committed JSON.
 *
 * @license MIT
 */
...
import { createCellular, METRIC_EUCLIDEAN, VERSION } from '../Cellular.js';

/** Seed for the committed euclidean golden. */
export const SEED = 42;

/** Jitter for the committed euclidean golden (the default). */
 ...
 * origin. Committed here so the digest is reproducible from source alone.
 */
export const COORDS = [
...
/** Build the golden record object (also used by the tests to compare). */
export function build() {
    const cell = createCellular(SEED, { metric: METRIC_EUCLIDEAN, jitter: JITTER });
    return {
        note: 'Committed euclidean cellular golden. Regenerate ONLY intentionally: node goldens/gen.mjs. A change is breaking (CHANGELOG note required).',
        version: VERSION,
        metric: 'euclidean',
        metricId: METRIC_EUCLIDEAN,
        seed: SEED,
        jitter: JITTER,
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
    const record = build();
    const path = join(dirname(fileURLToPath(import.meta.url)), 'euclidean.json');
